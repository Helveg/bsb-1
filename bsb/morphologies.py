...
def _selfer_method(cls, f, arglambda):
    def wrapper(self, *args, **kwargs):
        return f(cls(*arglambda(self)), *args, **kwargs)

    return wrapper


def _selfer_class(cls, arglambda=lambda self: self):
    # Fun little thingy that recreates the public interface of one class on another class
    # Usefull when X.f(...) == Y(x).f(...) eg:
    # Branch.rotate == SubTree([branch]).rotate
    # The transformation of `self` to `[self]` in this example can be done by passing
    # `arglambda`
    # This factory method produces an intermediate shim for the child class to inherit
    # from. I guess I could've also written a decorator? But I wanted to see if this would
    # work ;)
    import inspect

    wrapped_dict = {
        key: _selfer_method(cls, f, arglambda)
        for key, f in vars(cls).items()
        if inspect.isfunction(f) and not key.startswith("_")
    }
    selfer = type("Self" + cls.__name__, (), wrapped_dict)
    return selfer


class Branch(_selfer_class(SubTree, lambda self: ([self],))):
    """
    ...
    def __init__(self, *args, labels=None):
        ...
        for v, vector in enumerate(type(self).vectors):
            setattr(self, vector, np.array(args[v], copy=False, dtype=float))
    ...
    def get_point(self, index):
        ...
        """
        me = vars(self)
        return np.array(tuple(me[v][index] for v in Branch.vectors[:3]))
